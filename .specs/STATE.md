...
- **Rodada 1 (`form-and-data-primitives`)**: ✅ concluída, mergeada em `main` (PR #10), publicada no npm (PR #11 `chore: version packages` mergeada).
- **Rodada 2 (`still-void-gaps-round-2`)**: ✅ concluída, verificada (PASS, 45/45 ACs) e **publicada**. PR #12 mergeada em `main` (2026-08-25), CodeRabbit 15/15 threads resolvidas, `release.yml` abriu e mergeou PR #11 `chore: version packages` — **npm em `3.0.0`**. Entregou `Icon` (server-safe), família client migrada para CSS `sv-*`, `AlertDialog`, `Button variant="accent"`, `Card` com `as`/`asChild`, `@still-void/ui/tailwind.css` (v4 CSS-first). Relatório: `.specs/features/still-void-gaps-round-2/validation.md`.
- **Rodada 3 (`still-void-gaps-round-3`)**: ✅ **concluída e verificada.** Fechou T25 (`displayName` real nas 6 famílias Radix-derivadas) e T26 (`Tabs` emite `sv-tabs`, CSS ganha `align-items: flex-start`) — achados agendados da Fase 3 da rodada 2. Specify (spec brief, Medium) → Execute inline (2 tasks, sem Design/Tasks formais) → Verifier independente **PASS** (7/7 ACs com evidência, 4/4 gates verdes — test/typecheck/build/lint:package —, 3/3 mutantes mortos pelo sensor). Relatório: `.specs/features/still-void-gaps-round-3/validation.md`. Achado do Verifier: 2 testes de contrato textual (`ui-tabs-classes.test.tsx`, `ui-tooltip-classes.test.tsx`) varriam **toda** string literal do arquivo-fonte em vez de só posições `className=`, quebrando com os novos literais de `displayName` — corrigidos para o padrão já usado em `ui-dialog-behavior.test.tsx`/`ui-alert-dialog.test.tsx` (mesmo regex escopado, pré-existente).
- **Branch**: `claude/still-void-gaps-round-3`, criada a partir de `origin/main` (não da branch antiga da rodada 2, já mergeada) — 3 commits à frente: `18d4660` (T25), `64fa362` (T26), `fa850b8` (docs). **PR #13 aberta contra `main`**: https://github.com/KalleoPinheiro/still-void/pull/13
- **Changesets prontos**: 2× `patch` (`radix-display-names.md`, `tabs-root-container.md`) — nenhuma mudança de comportamento visível para quem já usa a API atual.
- **Próximo passo**: usuário decide quando mergear PR #13. Depois do merge, mesmo fluxo das rodadas 1 e 2 — `release.yml` abre PR `chore: version packages`, publica no npm quando essa PR mergear.
- **Blockers**: nenhum.
