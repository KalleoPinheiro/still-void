...
- **Rodadas 1–3** (`form-and-data-primitives`, `still-void-gaps-round-2`, `still-void-gaps-round-3`): ✅ todas concluídas, mergeadas em `main` (PR #10, #12, #13) e **publicadas no npm** (`3.0.0` → `3.0.1` via PRs de `chore: version packages`, última delas PR #14, já mergeada). Sem trabalho pendente dessas rodadas.
- **Branch atual**: `claude/design-system-critique-9143fb`, criada a partir de `origin/main` em `5244dfc` (topo do repo após a rodada 3). Aberta por um `/impeccable critique` na biblioteca inteira, não por uma feature específica.
- **`/impeccable critique`**: rodou dual-agent (design review + detector/browser) sobre `src/react` + Storybook. Score **30/40 (Good)**. Relatório: `.impeccable/critique/2026-08-25T17-23-10Z__src-react-stories.md`. 1 P0, 2 P1, 2 P2 identificados; usuário pediu pra atacar tudo, na ordem do relatório, incluindo o gap de nav mobile.
- **5 ações `/impeccable` executadas em sequência (harden → polish → adapt → document → polish)** — **validadas (test/typecheck/build/lint:package verdes a cada etapa) mas AINDA NÃO COMMITADAS** (não fizeram parte de um fluxo tlc-spec-driven; commit não foi pedido nessa fase). Mudanças em: `src/recipes/field.ts` (+`fieldMessage()`), `src/css/style.css` (`.sv-field[aria-invalid]`, `.sv-field-message`, nav-disclosure do Header), `DESIGN.md` (§5 error state, §5 nav mobile, frontmatter `#6D6D76`, `translateY: 2px`), `docs/design-system.md`, `src/lib/utils.ts` (comentário cx/cn), `src/react/components/Shell.tsx` (`<details>`/`<summary>` no nav), `src/react/stories/{Alert,Button,Input}.stories.tsx`, `src/recipes/shell.ts`, e os testes `tests/{field-css-contract,react-barrel,react-components,recipes-field}.test.ts` + novo `tests/header-nav-contract.test.ts`. 5 changesets prontos em disco, **também não commitados**: `warm-lemons-flow.md`, `tame-poems-relax.md`, `plenty-cows-jam.md`, `gentle-actors-nap.md`, `six-buses-argue.md`.
- **`storybook-token-cleanup` (via tlc-spec-driven, a pedido explícito do usuário)**: ✅ **concluída e verificada.** Fechou os 5 achados residuais do detector nas stories (`CategoryPill` raw-color como exceção documentada inline; 4 literais fora de escala em `Icon`/`Select`/`Tooltip` trocados por tokens reais). Specify (spec brief, Medium, `.specs/features/storybook-token-cleanup/spec.md`) → Execute inline (2 tasks, sem Design/Tasks formais) → Verifier independente **PASS** (8/8 ACs com evidência, 4/4 gates verdes — vitest/typecheck/detect.mjs/storybook build —, 2/2 mutantes mortos pelo sensor). Relatório: `.specs/features/storybook-token-cleanup/validation.md`. Único achado do Verifier (changeset `four-plums-hunt.md` fora do diff revisado) corrigido na hora em `218befc`. **Esses 4 commits (`13fcce5`, `756c579`, `218befc`, `2bc97ce`) já estão na branch.**
- **Changesets prontos**: 6× (5 da leva harden/.../polish, ainda não commitados + `four-plums-hunt.md` da leva `storybook-token-cleanup`, já commitado em `218befc`) — todos `patch` exceto `plenty-cows-jam.md` e `warm-lemons-flow.md` (`minor`, por exportarem `fieldMessage`/`headerClasses.navToggle`/`navSummary`).
- **Próximo passo**: perguntar ao usuário se commita a leva harden/.../polish (working tree ainda sujo com essas mudanças) antes de abrir PR contra `main`. Depois disso, mesmo fluxo das rodadas anteriores — abrir PR, `release.yml` cuida da versão/publish no merge.
- **Blockers**: nenhum técnico. Decisão pendente do usuário: commitar (ou não) o trabalho de harden/polish/adapt/document ainda solto no working tree.
