...
- On `(width < 40rem), (pointer: coarse)`, `.sv-btn` gets `min-height` and `min-width` of 44px, `.sv-field` and `.sv-tabs__trigger` get `min-height: 44px`, a `<label>` wrapping a `Checkbox` gets `min-height: 44px` (the checkbox keeps its size; the label is the target), and `.sv-tabs__list` grows to fit its triggers and scrolls horizontally inside itself instead of widening the page. This meets WCAG 2.5.5 touch targets. Pointer-fine desktop keeps the current sizes.
