...
- **Tailwind theme (optional):** `@still-void/ui/tailwind.css` — a Tailwind v4 `@theme inline` block — maps the same `sv-*` keys to `var(--sv-*)` for consumers composing their own markup with Tailwind utilities on top of the system's tokens. `inline` matters: without it a utility resolves `var(--sv-*)` once at build time and stops following `[data-theme]`/`[data-accent]`. It is not required by any component the package ships, and `tailwindcss` is declared as an optional peer dependency (`>=4`) accordingly — Tailwind v3 support, preset included, ended at `3.0.0`.
